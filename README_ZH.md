...
<p align="center">
  ...
  <a href="#路线图"><img src="https://img.shields.io/badge/sages-Li%20Bai%20✅%20|%20Laozi%20✅%20|%20Confucius%20✅%20|%20Du%20Fu%20✅-green" alt="Sages"></a>
</p>
...
| | 文件 | 受众 | 特色 |
|---|---|---|---|
| 🏮 | `SKILL.md` | 所有用户 | 李白、老子、孔子、杜甫的主路由入口 |
...
指令格式：`::先贤 [语言] [注解] [模式]`。「李白」四种写法等效：`::lb` / `::libai` / `::Li Bai` / `::李白`。「老子」四种写法等效：`::lz` / `::laozi` / `::Laozi` / `::老子`。「孔子」四种写法等效：`::kz` / `::confucius` / `::Confucius` / `::孔子`。「杜甫」四种写法等效：`::df` / `::dufu` / `::Du Fu` / `::杜甫`。
